...
load(here("0_DataPrep", "Data", "inc_data_OsteoDx_in_breastAI.RData"))

# Periods-----------------

IR.age <- inc_data_OsteoDx_in_breastAI
...
write.csv(ir_ci_all, file=here("3_IRR", db.name, "OsteoDxBreast",  "Observed_incidence_rates_OsteoDx_breastAI.csv"))
save(ir_ci_all, file=here("3_IRR", db.name, "OsteoDxBreast",  "Observed_incidence_rates_OsteoDx_breastAI.RData"))
...
write.csv(ir.post_ci, file=here("3_IRR", db.name, "OsteoDxBreast",  "Observed_incidence_rates_post_lockdown_OsteoDx_breastAI.csv"))
save(ir.post_ci, file=here("3_IRR", db.name, "OsteoDxBreast",  "Observed_incidence_rates_post_lockdown_OsteoDx_breastAI.RData"))
...
write.csv(ir_ci_pre_post, file=here("3_IRR", db.name, "OsteoDxBreast",  "Observed_incidence_rates_pre_post_lockdown_OsteoDx_breastAI.csv"))
save(ir_ci_pre_post, file=here("3_IRR", db.name, "OsteoDxBreast",  "Observed_incidence_rates_pre_post_lockdown_OsteoDx_breastAI.RData"))
...
Pretty_observed_IR_results_table <- flextable(ir_ci_pre_post_pivot) %>% theme_vanilla() %>% 
  set_caption(caption = "Incidence rates of treatment-related outcomes in breast cancer patients on aromatase inhibitors in each of the time periods, stratified by age") %>% 
  width(width = 1.4) 

save(ir_ci_pre_post_pivot, file=here("3_IRR", db.name, "OsteoDxBreast",  "Observed incidence_rates_ir_ci_pre_post_pivot_OsteoDx_breastAI.RData"))
write.csv(ir_ci_pre_post_pivot, file=here("3_IRR", db.name, "OsteoDxBreast",  "Observed incidence_rates_ir_ci_pre_post_pivot_OsteoDx_breastAI.csv"))

save_as_docx('Pretty_observed_IR_results_table' = Pretty_observed_IR_results_table, path=here("3_IRR", db.name, "OsteoDxBreast",  "Observed incidence_rates_OsteoDx_breastAI.docx"))
...
#### Save IRR
write.csv(rate_ratios_table_formatted, file=here::here("3_IRR", db.name, "OsteoDxBreast",  "Observed IRR_OsteoDX_BreastAI.csv"))
save(rate_ratios_table_formatted, file=here::here("3_IRR", db.name, "OsteoDxBreast",  "Observed IRR_OsteoDX_BreastAI.RData"))

#### Make pretty table for breast cancer

Pretty_IRR_table_OsteoDXBreastAI<- flextable(rate_ratios_table_formatted) %>% theme_vanilla() %>% 
  set_caption(caption = "Incidence rate ratios of treatment-related outcomes in breast cancer patients on aromatase inhibitors over the lockdown periods compared to pre-COVID period, stratified by sex") %>% 
  width(width = 1.4) 

save_as_docx('Pretty_IRR_table_OsteoDXBreastAI' = Pretty_IRR_table_OsteoDXBreastAI, path=here("3_IRR", db.name, "OsteoDxBreast",  "Pretty_IRR_table_OsteoDXBreastAI.docx"))
...
IRR_forest_osteoDx_Breast_AI =
  ...
  coord_flip()+
  ggtitle("Incidence Rate Ratios of Endocrine Treatment-Related Outcomes in Breast Cancer Patients on Aromatase Inhibitors Across Lockdown Periods")+
...
ggsave(here("3_IRR", db.name, "OsteoDxBreast",  "IRR_OsteoDX_BreastAI_forest.jpg"), IRR_forest_osteoDx_Breast_AI, dpi=600, scale = 1,  width = 12, height = 10)
