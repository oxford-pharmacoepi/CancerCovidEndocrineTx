...
for(i in seq_along(IncTxBreast_overall$outcome_cohort_id) ){
  ...
  working_table <- bind_rows(
    ...
    working_participants %>%
      summarise(val = as.character(paste(quantile(age,probs=0.25),  " to ",
                                           quantile(age,probs=0.75))))  %>%
      mutate(var="IQR age"),

    working_participants %>% 
      summarise(val = as.character(round(mean(age),2)))  %>% 
      mutate(var="Mean age"),
    
    working_participants %>% 
      summarise(val = as.character(round(sd(age),2)))  %>% 
      mutate(var="SD age"),
    ...
    working_participants %>% 
      summarise(val = as.character(round(sd(charlson),2)))  %>% 
      mutate(var="sd Charlson Index"),
    working_participants%>% 
      filter(sex == "Male") %>% 
      summarise(val = as.character(n()))  %>% 
      mutate(var="N male"),
