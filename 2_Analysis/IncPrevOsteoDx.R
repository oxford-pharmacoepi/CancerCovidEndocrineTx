...
## ============== SET THE BREAST CANCER TREATMENT STRATA DENOMINATOR COHORTS ========== ##
...
cdm <- generateDenominatorCohortSet(
  ...
  strataCohortId = 1,
  strataCohortName = "BreastCancerWithAromataseInhibitors(noTAM)",
...
cohortCount(cdm$BreastCancerWithAromataseInhibitors(noTAM)) # CHECK NAME OF OUTCOME COHORT IN TABLE AS MIGHT NOT LIKE BRACKETS

cohortSet(cdm$BreastCancerWithAromataseInhibitors(noTAM)) 
...
print(paste0("- Getting incidence treatment related outcomes in breast cancer populations"))
info(logger, "- Getting incidence treatment related outcomes in breast cancer populations")


IncTxOutcomesBreast <- estimateIncidence(
  cdm = cdm,
  denominatorTable = "BreastCancerStrata",
  outcomeTable = outcome_table_name_2, 
  outcomeCohortId = c(1:5), # add a filter here to specify which outcome cohorts to focus on specific to breast cancer
...
save(IncTxOutcomesBreast, file = here("Results", db.name, "3_OsteoDx", "IncTxOutcomesBreast.RData"))
...
exportIncidencePrevalenceResults(resultList=list("incidence" = IncTxOutcomesBreast), 
                                 zipName=paste0(db.name, "IncTxOutcomesBreast"),
...
inc_yrs_plot <- IncTxOutcomesBreast %>%  
...
inc_yrs_plot <- 
  ...
  ggtitle("Incidence Rates of Endocrine Treatment-Related Outcomes in Years in Breast Cancer Patients Before and After COVID-19 Lockdown") +
...
ggsave(here("Results", db.name , "3_OsteoDx", paste0(EndoTxBreastCancer_inc_yrs_plot, ".jpg")), inc_yrs_plot, dpi=600, scale = 1, width = 12, height = 9)
...
inc_months_plot <- IncTxOutcomesBreast %>%  
  filter(denominator_cohort_id == 1) %>%
  filter(analysis_outcome_washout == NULL) %>% 
...
inc_months_plot <- 
  ...
  geom_errorbar(width=0) +
  scale_y_continuous(limits = c(0, 150)) +
  ggtitle("Incidence Rates of Endocrine Treatment-Related Outcomes in Months in Breast Cancer Patients Before and After COVID-19 Lockdown") +
  labs(colour = "Endocrine Treatment", x="Time" , y="Incidence per 100000 person-years") +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  geom_vline(xintercept=as.numeric(as.Date(c("2020-03-23"))),linetype=2, color="red")
...
# Save the plot as jpg
ggsave(here("Results", db.name , "3_OsteoDx", paste0(EndoTxBreastCancer_inc_months_plot, ".jpg")), inc_months_plot, dpi=600, scale = 1, width = 12, height = 9)
...
inc_qrs_plot <- IncTxOutcomesBreast %>%  
  filter(denominator_cohort_id == 1) %>%
  filter(analysis_outcome_washout == NULL) %>% 
...
inc_qrs_plot <- 
  ...
  geom_errorbar(width=0) +
  scale_y_continuous(limits = c(0, 150)) +
  ggtitle("Incidence Rates of Endocrine Treatment-Related Outcomes in Quarters in Breast Cancer Patients Before and After COVID-19 Lockdown") +
  labs(colour = "Endocrine Treatment", x="Time" , y="Incidence per 100000 person-years") +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  geom_vline(xintercept=as.numeric(as.Date(c("2020-03-23"))),linetype=2, color="red")
...
# Save the plot as jpg
ggsave(here("Results", db.name , "3_OsteoDx", paste0(EndoTxBreastCancer_inc_qrs_plot, ".jpg")), inc_qrs_plot, dpi=600, scale = 1, width = 12, height = 9)
